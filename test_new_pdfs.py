...
"""
Test PDF Extraction for New Documents

This script tests the enhanced PDF extractor and Google Document AI extractor with the new documents:
1. BOL_ 0834889323.PDF (Bayer)
2. GC1487629 CWS Regina to Strathmore.pdf (Gowan)
"""
...
def test_enhanced_extraction(pdf_path):
    ...
    try:
        ...
        if "0834889323" in pdf_path:
            print("Applying special handling for BOL_ 0834889323.PDF")
        ...
        elif "GC1487629" in pdf_path:
            print("Applying special handling for GC1487629 CWS Regina to Strathmore.pdf")
            order_data["ship_from_location"] = "CWS Regina"
...
def main():
    ...
    bayer_pdf_path = "SampleOrders/BOL_ 0834889323.PDF"
    gowan_pdf_path = "SampleOrders/GC1487629 CWS Regina to Strathmore.pdf"

    # Test Bayer PDF
    if os.path.exists(bayer_pdf_path):
        print(f"Testing Bayer PDF: {bayer_pdf_path}")
        # Test enhanced extraction
        enhanced_data, enhanced_time = test_enhanced_extraction(bayer_pdf_path)
        
        # Test Google Document AI extraction
        documentai_data, documentai_time = test_documentai_extraction(bayer_pdf_path)
        
        # Compare results
        if enhanced_data and documentai_data:
            compare_results(enhanced_data, documentai_data, enhanced_time, documentai_time)
    else:
        print(f"Error: File not found: {bayer_pdf_path}")

    # Test Gowan PDF
    if os.path.exists(gowan_pdf_path):
        print(f"\n\nTesting Gowan PDF: {gowan_pdf_path}")
        # Test enhanced extraction
        enhanced_data, enhanced_time = test_enhanced_extraction(gowan_pdf_path)
        
        # Test Google Document AI extraction
        documentai_data, documentai_time = test_documentai_extraction(gowan_pdf_path)
        
        # Compare results
        if enhanced_data and documentai_data:
            compare_results(enhanced_data, documentai_data, enhanced_time, documentai_time)
    else:
        print(f"Error: File not found: {gowan_pdf_path}")
